# Objective: composition analyses through time according to land use
...
# read in dataset
invertsfam <- read.csv("./data/invertsfam.csv", row.names = 1, header = T)

# remove experimental unit identifiers to get community matrix
invertcommF2 <- invertsfam %>%
  dplyr::select(-(Agency:PRE_POST))

# Remove singleton and doubleton families (appear in less than 3 experimental units)
invertcommF2 <- invertcommF2[,(colSums(invertcommF2) > 2)]
...
Ecoregion <- invertsfam$Ecoregion_NARS
NLCD <- invertsfam$landuse
Year <- invertsfam$Year
Agency <- invertsfam$Agency
PRE_POST <- invertsfam$PRE_POST
...
response = vegdist(invertcommF2, dist = "bray")

##the Condition() means that this is a partial dbRDA; such that the influence of
##Agency, Ecoregion, and PRE_POST are accounted for, before we constrain
...
#           Df SumOfSqs       F Pr(>F)    
#NLCD        3    4.952 22.7172 0.0001 ***
#Year        1    0.465  6.3935 0.0001 ***
#NLCD:Year   3    0.377  1.7288 0.0165 *  
#Residual  518   37.635                                    
...
modA
##Explains 44.58% of variance ## 1-0.5542 

summary(modA)$concont$importance[,c(1:5)]
##dbRDA1 explains 76.1% of fitted variance
##dbRDA2 explains 9.3% of fitted variance

summary(modA)$cont$importance[,c(1:5)]
##dbRDA1 explains 10.1% of total variance
...
##Generate the 95% confidence interval ellipses for the centroid of each landuse 
...
A <- ggplot()+
  ...
  geom_path(data = df_ell,
            aes(x = dbRDA1, y = dbRDA2, color = Landuse), size=1) +
  geom_point(data = centr,
            aes(x = dbRDA1, y = dbRDA2, color = Label), size=1) +
  ##Vectors
  ggrepel::geom_text_repel(data = bpvecs,
                           ...
                           bg.color = "white",
                           bg.r = .1)+
  xlab("dbRDA1 (76.1% of fitted, 10.1% of total variation)") +
  ylab("dbRDA2 (9.3% of fitted, 1.2% of total variation)")+
...
invertcommF3 <- sqrt(decostand(invertcommF2, method = "hellinger"))
...
en1F <- envfit(modA ~ . , data = invertcommF3, permutations = 0)
...
# match the Taxa (families) in genscoresF to Families in invertgenmain
...
# so, for those NA's match and pull the subfamily info from invertgenmain
...
genscoresF$veclength <- sqrt(genscoresF$dbRDA1^2 + genscoresF$dbRDA2^2)

B <- genscoresF %>% 
  filter(veclength > 0.3) %>%
  ggplot()+
  geom_hline(yintercept = 0, linetype = "dashed")+
  geom_vline(xintercept = 0, linetype = "dashed")+
  ##Note: r is the radius of the circle and needs to be set to the scaling value
...
ggsave("./figures_maintext/composition_prepost.png", f2_all, dpi = 200, 
       height = 8, width = 12)